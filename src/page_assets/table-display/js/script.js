...
function get_table_headers(header_rows) {
    ...
    const tr2 = get_cl_element("tr", "lovRow border-b bg-card", null, null)
    for (let hd of header_rows) {
        col_name_types[hd[0]] = hd[1]
        let th = get_cl_element("th", "px-3 py-2 font-medium text-card-foreground text-left", null, null)
        ...
        if (primary_column && header_rows.indexOf(hd) == 0) {
            let el = get_cl_element("input", "input", "selectAll", null)
...
function add_insert_row() {
    let tr = get_cl_element("tr", "insert border-r")
    let input_el
    for (const [idx, col_name] of col_names.entries()) {
        let autofill_flag = false
        let td = get_cl_element("td","px-3 py-2 border-r")
        tr.appendChild(td)
        if (idx == 0) {
            td.id = 0
        }
